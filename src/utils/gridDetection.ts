 ...
 */
export const detectGridLines = async (
  imageElement: HTMLImageElement
): Promise<GridSuggestion | null> => {
  try {
    ...
    const scale = Math.min(
      1,
      maxDim / Math.max(imageElement.width, imageElement.height)
    );
    ...
    const verticalEdges = detectEdges(
      data,
      canvas.width,
      canvas.height,
      "vertical"
    );
    const horizontalEdges = detectEdges(
      data,
      canvas.width,
      canvas.height,
      "horizontal"
  ...
  if (direction === "vertical") {
    for (let x = 1; x < width - 1; x++) {
      ...
      for (let y = 0; y < height; y++) {
        ...
        // Check bounds
        if (
          data[nextIdx] === undefined ||
          data[prevIdx] === undefined ||
          data[nextIdx + 1] === undefined ||
          data[prevIdx + 1] === undefined ||
          data[nextIdx + 2] === undefined ||
          data[prevIdx + 2] === undefined
        ) {
          continue;
        }
